...
from django.urls import (
    RegexURLResolver, RegexURLPattern, URLResolver, URLPattern, RoutePattern
)
...
def recursion_urls(patterns, pre_fix, url_ordered_dict, pre_namespace):
    for item in patterns:
        ...
        if isinstance(item, URLPattern):  # URL匹配对象
            ...
            item_name = item.name
            if not item_name:
                item_name = '%s:%s' % (pre_namespace, pre_fix + part)

            url_ordered_dict[item_name] = {
                'name': item_name,
                'url': pre_fix + part,
